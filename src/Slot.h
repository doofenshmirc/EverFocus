...
class SlotClass {
public:
  SlotClass(uint16_t addr, uint8_t slot_id);

  uint8_t getId() { return _slot_id; }
  void setId(uint8_t slot_id) { _slot_id = slot_id; }

  uint16_t getAddress() { return _addr; }
  void setAddress(uint16_t addr) { _addr = addr; }

  uint8_t getStatus() { return _slot_status & SLOT_STAT_BIT; }
  void setStatus(uint8_t status) { _slot_status = (_slot_status & ~SLOT_STAT_BIT) | status; }
